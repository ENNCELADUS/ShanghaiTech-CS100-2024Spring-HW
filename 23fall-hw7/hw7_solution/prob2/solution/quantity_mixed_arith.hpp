...
public:
  ...
  template <typename U>
  constexpr Quantity &operator+=(const Quantity<U, Dim> &rhs) {
    mValue += rhs.value();
    return *this;
  }
  ...
  template <typename U>
  constexpr Quantity &
  operator*=(const Quantity<U, Dimensions<0, 0, 0, 0, 0, 0, 0>> &scalar) {
    mValue *= scalar.value();
    return *this;
  }
  ...
  template <typename U>
  constexpr auto operator+(const Quantity<U, Dim> &rhs) const {
    return Quantity<decltype(value() + rhs.value()), Dim>(value() +
                                                          rhs.value());
  }
...
template <typename T, typename U, int... D1, int... D2>
constexpr auto operator*(const Quantity<T, Dimensions<D1...>> &lhs,
                         const Quantity<U, Dimensions<D2...>> &rhs) {
  return Quantity<decltype(lhs.value() * rhs.value()),
                  Dimensions<(D1 + D2)...>>(lhs.value() * rhs.value());
}
