...
void hide_cursor(void) { wprintf(L"\033[?25l"); }

void show_cursor(void) { wprintf(L"\033[?25h"); }
...
void sigint_handler(int sig) { sig_status = sig; }
...
int is_terminal_size_small(int prows, int pcols)
{
#ifdef _WIN32
    ...
    if (csbi.srWindow.Bottom < prows + 3 || csbi.srWindow.Right < pcols * 2)
    {
    ...
    if (w.ws_row < prows + 3 || w.ws_col < pcols * 2)
    {
        wprintf(L"Terminal size is small than %d * %d. Please enlarge the terminal.\n", prows + 3, pcols * 2);
        return 1;
    }
...
void clear_screen(void) { wprintf(L"\033[2J\033[1;1f"); }
